      ...
      try {
        ...
        const enriched = rawCinemas.map((cinema, idx) => {
          ...
          return {
            ...cinema,
            distanceText:
              distanceData.rows[0].elements[idx]?.distance?.text || "❓",
            photoUrl,
          };
        });
        setCinemas(enriched);
  ...
  />
      ...
      <div>
        ...
        <div className="overflow-x-auto flex gap-4 pb-2 snap-x snap-mandatory scroll-smooth hide-scroll">
          {(searchResults.length > 0 ? searchResults : movies.slice(0, 10)).map(
            (movie) => (
              <div
                key={movie.id}
      ...
      <div>
        ...
        <div className="space-y-2">
          {(showAll ? cinemas : cinemas.slice(0, 2)).map((cinema, idx) => {
            const formattedDistance = cinema.distanceText
              .replace("كم", "km")
              .replace(",", ".");

            const closingTime =
              cinema.opening_hours?.weekday_text?.find((line) =>
                line.toLowerCase().startsWith("monday")
              ) || "";

            const closingText =
              closingTime.split(": ")[1] || "Closed time not available";
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <a
                    ...
                    className="block text-xs text-blue-400 hover:underline w-fit"
                  >
                    {formattedDistance}
                  </a>
                  <p className="text-xs text-gray-400">
